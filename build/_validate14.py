# -*- coding: utf-8 -*-
"""Validate story 14 build output."""
import re, json, os
...
s14 = next(s for s in root["stories"] if s["id"] == SID)
if len(s14["paras"]) != N:
    errs.append("stories.js paras=%d != %d" % (len(s14["paras"]), N))
if s14["title_zh"] != st["title_zh"]:
...
if me is None:
    errs.append("manifest missing id 14")
elif len(me["paras"]) != N:
