# -*- coding: utf-8 -*-
"""Validate story 15 build output."""
import re, json, os
...
s15 = next(s for s in root["stories"] if s["id"] == SID)
if len(s15["paras"]) != N:
    errs.append("stories.js paras=%d != %d" % (len(s15["paras"]), N))
if s15["title_zh"] != st["title_zh"]:
...
if me is None:
    errs.append("manifest missing id 15")
elif len(me["paras"]) != N:
