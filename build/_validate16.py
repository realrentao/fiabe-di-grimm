# -*- coding: utf-8 -*-
"""Validate story 16 build output."""
import re, json, os
...
s16 = next(s for s in root["stories"] if s["id"] == SID)
if len(s16["paras"]) != N:
    errs.append("stories.js paras=%d != %d" % (len(s16["paras"]), N))
if s16["title_zh"] != st["title_zh"]:
...
if me is None:
    errs.append("manifest missing id 16")
elif len(me["paras"]) != N:
